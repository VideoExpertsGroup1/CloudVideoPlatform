window.SkyVR = new function (){
	this.description = "API Lib For CloudAPI. Network Layer Between FrontEnd And BackEnd.";
	this.config = {
		url: "",
		url_cameras: "",
		url_api: "",
	...
	this.setURL = function(url){
		if(this.config.url != url){
			...
			this.config.url_admin_cameras = url+"api/v2/admin/cameras/";
	...
	this.changeLocationState = function(newPageParams){
		...
		if(this.containsPageParam("customswf"))
			params.push('customswf=' + encodeURIComponent(this.pageParams['customswf']))
		if(this.containsPageParam("hls"))
			params.push('hls=' + encodeURIComponent(this.pageParams['hls']))
...
/*	this.getUTC = function(camtimezone){
	...
	this.accountInfo = function(){
		var d = $.Deferred();
		console.log(CloudAPI.config.url_account);
		$.ajax({
	...
	this.storageDataFirstRecord = function(startDT){
		var d = $.Deferred();
		var request_data = {
	...
	this.camerasList = function(){
		...
		getData(request_data).fail(function(r){
			d.reject(r);
		}).done(function(data){
			result.meta.total_count = data.meta.total_count;
			$.merge(result.objects, data.objects);
...
CloudAPI.hostNewCamera = function(data){
	return $.ajax({
		url: CloudAPI.config.url_cameras + "host_new_camera/",
		type: 'POST',
		data: JSON.stringify(data),
		contentType: 'application/json',
		cache : false
