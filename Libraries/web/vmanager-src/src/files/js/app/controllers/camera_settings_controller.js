define(['config', 'backbone','underscore', 'application', '../views/camera_settings_view','event', 'jquery-ui'], function(conf, bb,_,app,cam_settings_view, event){
	var application = {
		show : function(){
			var self = this;
			...
			cam_settings_view.show().done(function(){
				...
				$('.settings-list-element.dropdown .element-name').click(dropdownHandler);
				$('.settings-list-element.new-window .element-name').click(newWindowHandler);
				$('form#camera_name').submit(function(e){
					e.preventDefault();
					if(CloudUI.isDemo()){CloudUI.showDialogDemo();return;}
					if(!CloudAPI.hasAccessSettings()) return;
				...
				$('form#camera_url_edit').submit(function(e){
					e.preventDefault();
					...
					setTimeout(function(){
						console.log("Restart player and switch to live!!!");
						event.trigger(event.PLAYER_FORCE_REINIT);
					},500);
					
				});

				$('#memory_card_refresh').click(function(e) {
					$('#memory_card_text').html(""); // for show throbber
					cam_settings_view.updateInformationAboutMemoryCard();
				});

				$('#memory_card_format').click(function(e){
					e.preventDefault();
					if(SkyUI.isDemo()){SkyUI.showDialogDemo();return;}
					if(!CloudAPI.hasAccessSettings()) return;
				...
				$('form#camera_record_mode input').click(function(){
					if(CloudUI.isDemo()){
						$("#camera_record_mode_" + CloudAPI.cache.cameraInfo().rec_mode).prop("checked", true);
				...
				$(".scheduler-power-button").click(function(){
					...
					CloudAPI.cameraInfo().done(function(data){
						...
						// TODO merge with CloudPlayer.togglePower from file CloudPlayer.js
