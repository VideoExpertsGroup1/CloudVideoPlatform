window.TimelineLoader = function (event){
	var self = this;
	self.event = event;
	this.description = "Class timeline loader for SkyVR";
	...
	this.startDataPoling = function(){
		...
		function polingFunc(){
			...
			// var startTime = lastEndTime ? (lastEndTime - polingTime) : (SkyVR.getCurrentTimeUTC() - 5*polingTime);
			var startTime = SkyVR.getCurrentTimeUTC() - 5*polingTime;
			if(!SkyVR.isP2PStreaming()){
				var startDT = SkyVR.convertUTCTimeToUTCStr(startTime);
				SkyVR.storageData(startDT).done(function(data){
					self.recordList.append(data.objects); // TODO make use done
				});
				SkyVR.storageEvents(startDT).done(function(data){
					self.eventList.append(data.objects); // TODO make use done
				});
				SkyVR.storageThumbnails(startDT).done(function(data){
					self.thumbnailList.append(data.objects); // TODO make use done
				});

				// poling firsts data
				SkyVR.storageDataFirstRecord().done(function(record){
				...
				SkyVR.storageEventsFirstRecord().done(function(record){
				...
				SkyVR.storageThumbnailsFirstRecord().done(function(thumb){
	...
	this.getRecordsLoading = function(){
		var result = [];
		var curTime = SkyVR.getCurrentTimeUTC();
	...
	this.getLaststData = function(){
		if(!this['lastsDataTime']){
			this['lastsDataTime'] = SkyVR.getCurrentTimeUTC();
		}
	...
	this.loadEventsPortion = function(i){
		...
		var self = this;
		var startDT = SkyVR.convertUTCTimeToUTCStr(i);
		var endDT = SkyVR.convertUTCTimeToUTCStr(i + this.durationGrid);
		// console.log("[TIMELEINELOADER] Loading events 3 hours of data from " + startDT);
		SkyVR.storageEvents(startDT, endDT).done(function(data){
	...
	this.loadThumbnailsPortion = function(i){
		...
		var startDT = SkyVR.convertUTCTimeToUTCStr(i);
		var endDT = SkyVR.convertUTCTimeToUTCStr(i + this.durationGrid);
		SkyVR.storageThumbnails(startDT, endDT).done(function(data){
			var result = data.objects;
	...
	this.loadRecordsPortion = function(i){
		...
		SkyVR.storageData(startDT, endDT).done(function(data){
			var result = data.objects;
	...
	this.load = function(start, end){
		...
		for(var i = start_grid; i <= end_grid; i = i + this.durationGrid){
			...
			}else if(SkyVR.isP2PStreaming() && SkyVR.hasMemoryCard()){
	...
	this.searchFirstRecord = function(t, def){
		...
		self.load(start,end).done(function(){
			...
			if(rec && rec.startTime <= end){
				...
				d.resolve(rec);
			}else if(end <= self.normalizeT(SkyVR.getCurrentTimeUTC() + self.durationGrid)){
				self.searchFirstRecord(end, d);
			}else{
				self.event.trigger(self.event.TIMELINE_LOADED);
				d.reject();
			}
	...
	this.searchNextRecord = function(t, def){
		...
		self.load(start,end).done(function(){
			...
			if(rec &&  rec.startTime <= end){
				self.event.trigger(self.event.TIMELINE_LOADED);
				d.resolve(rec);
			}else if(end < SkyVR.getCurrentTimeUTC()){
