                ...
                getStructure: function () {
                    return {
                        ...
                        getLocalDate: function (e) {
                            i = new Date(e);
                            return Date(i.getUTCFullYear(), i.getUTCMonth(), i.getUTCDate(), i.getUTCHours(), i.getUTCMinutes(), i.getUTCSeconds())
                        }
                    }
                }
...
define('timeline-model', ['backbone', 'transport'], function (backbone, t) {
    var model = backbone.Model.extend({
        defaults: {
            isBuffering: 0,
            isLoaded: 0,
            time: new Date(SkyVR.getCurrentTimeUTC()),
            isLive: 0,
            isPlaying: 1,
            volume: 0.5
        },
        initialize: function (e, t) {
            var i = t.camera;
            this.structure = i.getStructure();
            this.listenTo(i, "change:where_id", function () {
                this.structure = i.getStructure()
            });
            this.on("change:volume", function (e, t) {
                t.cookie.setCookieValue(s, t)
            })
        },
        getTime: function (e) {
            // return new Date(this.structure.getLocalDate(e || new Date(1e3 * this.get("time"))))
            return new Date(e || this.get("time"));
        }
    }, this);
    m = new model(this, t);
    return m;
...
define('timeline', ['backbone', 'config', 'transport', 'underscore', 'views/home/view', "tmpl/main/timeline", 'd3', 'timeline-model', 'calendar', 'event', 'timezonejs'], function (backbone, config, transport, underscore, view, timeleineTpl, d3, m, calendar, event, timezoneJS) {
    ...
    function appendMilseconds(time) {
        return time * 1e3
    }
    ...
    underscore.extend(obj, {
        ...
        events: {},
        videos: [
            'http://54.173.34.172:8000/web/video/233857.mp4',
            'http://54.173.34.172:8000/web/video/152040.mp4',
            'http://54.173.34.172:8000/web/video/233857.mp4',
		    'http://54.173.34.172:8000/web/video/152040.mp4'
        ],
        initialize: function (param) {
			...
			if(!this.defaultDuration){
				active_level = $($('.zoom-levels span')[1]);
				this.defaultDuration = parseInt(active_level.attr('duration'),10);
				this.defaultDurationScale = parseInt(active_level.attr('scale'),10);
			}

			this.duration = parseInt(active_level.attr('duration'),10);
			this.durationScale = parseInt(active_level.attr('scale'),10);
            // reset thumbnails
            ...
            this.dateEnd = SkyVR.getCurrentTimeUTC();
            ...
            event.trigger(event.GET_CAMERA, function (camera) {
                ...
                if (self.cam.p2p_streaming && self.cam.p2p_streaming == true && !self.cam.p2p) {
					SkyVR.cameraP2PSettings(camera.id, function(response){
			...
			$('.button-clip-save').unbind().click(function(){
				...
				if(found){
					...
					startTime = d.getTime() - SkyVR.getOffsetTimezone();
					start = SkyVR.convertUTCTimeToUTCStr(startTime);
				}else{
				...
				if(found){
					...
					d.setUTCMinutes(parseInt(found[5], 10));
					d.setUTCSeconds(parseInt(found[6], 10));
					stopTime = d.getTime() - SkyVR.getOffsetTimezone();
					stop = SkyVR.convertUTCTimeToUTCStr(stopTime);
				}else{
				...
				SkyVR.serverTime().done(function(res){
					if(cc.clip_creation_delete_after != null){
						...
						delete_at = SkyVR.convertUTCTimeToUTCStr(delete_at);
						console.log('delete_at: ' + delete_at);
					}
					SkyVR.storageClipCreate(title, group, start, stop, delete_at).fail(function(jqXHR, textStatus, errorThrown){
			...
			event.on(event.TOGGLE_STREAMING, function (e) {
				...
				SkyVR.cameraInfo().done(function(response){
					this.cam = response;
			...
			event.on(event.TIMELINE_PORTION_DATA_LOADED, function (e) {
				if(self.realDomainUTC){
					// console.log("TIMELINE_PORTION_DATA_LOADED", arguments);
					if(arguments[0] < self.realDomainUTC[1] && arguments[1] > self.realDomainUTC[0]){
						// console.log("TIMELINE_PORTION_DATA_LOADED draw");
						self.drawRecordsLoading();
						self.drawEventsLoading();
						self.drawRecords();
						self.drawEventPoints();
						if(self.model.get('time') < self.timelineLoader.firstDataRecordTime){
            ...
            event.on(event.PLAYER_TIME_CHANGED, function (time) {
                ...
                var old_position = self.model.get('time');
                ...
                if (el) {
					// visualization time
					if(!time) time = SkyVR.getCurrentTimeByCameraTimezone();
                ...
                var endTime = self.realDomainUTC[1];
                // console.log("endTime=", endTime);
                var t = self.dimensions;
                var d = self.getDuration();
                // console.log("d=", d);
                // console.log("t.visibleTimelineWidth=", t.visibleTimelineWidth);
                // console.log("t.timelineWidth=", t.timelineWidth);
                var i = (d * (t.visibleTimelineWidth / t.timelineWidth));
                // console.log("i=", i);
                i -=  i*0.05;
                // console.log("i(2)=", i);
                var p = (endTime - time) * 100 / (endTime - (endTime - i));
                // console.log("p=", p);
                if (p <= 5 && p > 0) {
                    var Domain = self.realDomainUTC;
					Domain[0] += i/2;
					Domain[1] += i/2;
                    if(!self.getLock()){
                ...
                } else if(!self.UserActivity.isActive && p < 0 && oldVisible != self.playheadVisible) {
					/*var Domain = self.timeScale.domain();
                    Domain = underscore.map(Domain, function (el) {
						return new Date((Date.parse(el) + i/2 + self.model.get('time') - old_position))
                    });*/
                    if(!self.getLock()){
                        self.setDomain(self._calcDomain());
                    }else{
						console.warn("Domain locked 2");
					}
                }
            });
            ...
            event.on(event.CALENDAR_DATE_CHANGED, function (data) {
				...
				t = t - SkyVR.getOffsetTimezone();
				var start = t - self.duration/2;
				var end = t + self.duration/2;
            ...
            this.listenTo(this.model, "change:time", function (e) {
				// TODO must be init only one time
                return this.drawPlayhead();
            }, this);
            this.jumpAheadCutoff = 0.75;
			this.realDomainUTC = this._calcDomain();
            this.timeScale = d3.time.scale().range([0, this.dimensions.timelineWidth]).domain([
				this.realDomainUTC[0],
				this.realDomainUTC[1]
            ]);

			// load first data
			if(!self.timelineLoader.isLoad(this.realDomainUTC[0], this.realDomainUTC[1])){
				$('.timeline-container').addClass("loading");
			}
			this.timelineLoader.load(this.realDomainUTC[0], this.realDomainUTC[1]).done(function(){
				$('.timeline-container').removeClass("loading");
			});
            ...
            $('.zoom-levels span').click(function (e) {
				...
				if(self.duration != duration){
					...
					var t = self._timeInDomain() ? self.model.get('time') || null : a[1] - ((a[1] - a[0]) /2);
            ...
            event.trigger(event.PLAYER_TIME_CHANGED, SkyVR.getCurrentTimeUTC());
            // this.setDomain(this._calcDomain())
        },
        UserActivity:{
            userActivityTO:null,
            isActive:false,
            setIsAcvtive:underscore.debounce(function(){var self = this; self.isActive = true; clearTimeout(self.userActivityTO); self.userActivityTO = setTimeout(function () {
                self.isActive = false;
            },3e3)},50)
        },
        dispose:function(){
			...
			this.durationScale = this.defaultDurationScale;
        ...
        getDataForTimeline: function (data,param1) {
            var localData = data;
            var timeRange = this.getRealDomain(this.getDuration()); //15 minutes overhead in data
            var start = timeRange[0] - this.duration;
            var end = timeRange[1] + this.duration;
            if(this.duration < 3600000){ // duration less than 1 hour
        ...
        sendPlayerRecords: function(a){
			...
			self.sessionWatchdog().removeListner();
				self.timelineLoader.searchFirstRecord(a.getTime()).done(function(obj1){
					...
					self.model.set({time:obj1.startTime});
        ...
        initializeAxes: function () {
            ...
            this.labelledAxis = d3.svg.axis().scale(this.timeScale).orient("bottom").ticks(10).tickSize(0).tickFormat(function (e) {
				...
				var scale = parseInt(self.durationScale / 60000,10);
				if(this.showUTC == undefined)
					this.showUTC = SkyVR.containsPageParam("timeline_in_utc");
				if(this.showUTC)
        ...
        initializeGroups: function () {
			...
			SkyVR.cameraPreview(SkyVR.cameraID()).done(function(previews){
			...
			function updateThumbnail(){
				...
				try{
					if(self.lastThumbnailPageX > 0){
						...
						var thumb = self.timelineLoader.searchThumbnail(t, self.durationScale/1000);
						if(thumb){
        ...
        initializeZoom: function () {
            ...
            this.zoom = d3.behavior.zoom().scaleExtent([1, 1]).x(this.timeScale).on("zoomstart", underscore.bind(function () {
				...
				self.realDomainUTC[0] = self.timeScale.domain()[0].getTime();
				self.realDomainUTC[1] = self.timeScale.domain()[1].getTime();
				self.setDomain(self.realDomainUTC);
                ...
                this.UserActivity.setIsAcvtive();
                this.drawAxes();
                this.drawRecords();
				...
				self.realDomainUTC[0] = self.timeScale.domain()[0].getTime();
				self.realDomainUTC[1] = self.timeScale.domain()[1].getTime();
				if(!self.timelineLoader.isLoaded(self.realDomainUTC[0],self.realDomainUTC[1])){
					$('.timeline-container').addClass("loading");
					self.setDomain(self.realDomainUTC);
					self.timelineLoader.load(self.realDomainUTC[0],self.realDomainUTC[1]).done(function(){
						$('.timeline-container').removeClass("loading");
					});
				};
            }, this));
        ...
        resizeEventHandler: underscore.debounce(function () {
			...
			var middlePoint = this.realDomainUTC[1] - d/2;
        ...
        isOutOfBoundsLeft: function () {
			var firstStartTime = this.timelineLoader.recordList.firstStartTime() || SkyVR.getCurrentTimeUTC();
        ...
        getRealDomain: function (delta) {
            ...
            var i = (this.getDuration() * (t.visibleTimelineWidth / t.timelineWidth));
            this.realDomainUTC = this.realDomainUTC || this._calcDomain(SkyVR.getCurrentTimeUTC());
            return [this.realDomainUTC[1] - i - delta, this.realDomainUTC[1] + delta];
        },
        ...
        drawThumbnail: function(thumb, x, time){
			...
			// $('.skyvr-timeline-thumbnail-time').text(SkyVR.convertUTCTimeToStr(Date.parse(thumb.time + "Z") + SkyVR.getOffsetTimezone()));
			$('.skyvr-timeline-thumbnail-time').text(SkyVR.convertUTCTimeToSimpleStr(time + SkyVR.getOffsetTimezone()));
        ...
        skipPage: function (e) {
            ...
            var n = "fwd" === e ? i : 0 - i;
            var tmp = [
				this.realDomainUTC[0] + n,
				this.realDomainUTC[1] + n
            ]
            /*this.realDomainUTC[0] += n;
            this.realDomainUTC[1] += n;*/
            ...
            this.timelineLoader.load(tmp[0], tmp[1]).done(function(){
				self.realDomainUTC[0] = tmp[0];
				self.realDomainUTC[1] = tmp[1];
				self.setDomain(self.realDomainUTC);
				$('.timeline-container').removeClass("loading");
			});
        },
        ...
        correctTime: function(t){ // must be timestamp
			...
			if(!this.timezoneOffset)
				this.timezoneOffset = SkyVR.getOffsetTimezone();
			if(!this.localOffset)
        ...
        setDomain: function (s) {
			...
			/*if (this.realDomainUTC[0] == s[0] && this.realDomainUTC[1] == s[1]){
				// already set this values
				return;
			}*/
            this.realDomainUTC[0] = s[0];
			this.realDomainUTC[1] = s[1];
			
			// correct domain right border
			var rigth_border = SkyVR.getCurrentTimeUTC() + this.duration*this.jumpAheadCutoff;
			if(this.realDomainUTC[1] > rigth_border){
				var diff = this.realDomainUTC[1] - rigth_border;
				this.realDomainUTC[0] = this.realDomainUTC[0] - diff - 60000;
				this.realDomainUTC[1] = this.realDomainUTC[1] - diff - 60000;
			};

			// correct domain left border
			if(this.timelineLoader && this.timelineLoader.firstDataRecordTime){
				this.calculateWidthOfTimelineContainer();
				var left_border = this.timelineLoader.firstDataRecordTime - this.duration*this.jumpAheadCutoff;
				if(this.realDomainUTC[0] < left_border){
					var diff = left_border - this.realDomainUTC[0];
					this.realDomainUTC[0] = this.realDomainUTC[0] + diff + 60000;
					this.realDomainUTC[1] = this.realDomainUTC[1] + diff + 60000;
				}
			}
            ...
            this.svg.transition().duration(1e3).tween("axis", underscore.bind(function () {
				...
				this.timelineLoader.load(tmp[0] - this.duration, tmp[1] - this.duration);
				this.timelineLoader.load(tmp[0] + this.duration, tmp[1] + this.duration);
				this.timelineLoader.load(tmp[0] - this.duration*2, tmp[1] - this.duration*2);
				this.timelineLoader.load(tmp[0] + this.duration*2, tmp[1] + this.duration*2);
            }, this));
        }
    });
