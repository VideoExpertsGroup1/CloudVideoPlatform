define(['application', 'config', 'backbone','underscore','event', 'moment-timezone','arcticmodal'], function(app, conf, bb,_, event, timezone){
	var application = {
		show : function(){
			...
			$('.camera-settings-popup .header p').text(app.polyglot.t('Camera settings'));
				...
				for(var i=0;i<settings_elements.length; i++){
					...
					switch(settings_elements[i]['type']){
						...
						case 'new_window':
							...
							break;
						case 'power_button':
							settings_list_element = _.template($('#templates #camera-settings-camera-toggle-element').html());
							break;
						case 'read_only':
