...
public class CmdStreamStart implements CmdHandler {
    public static final String TAG = CmdStreamStart.class.getSimpleName();

    @Override
    public String cmd() {
        return CameraManagerCommandNames.STREAM_START;
    }
    ...
    public void handle(JSONObject request, CameraManagerClientListener client) {
        ...
        try {
            ...
            if(cam_id != client.getConfig().getCamID()){
                ...
                client.sendCmdDone(cmd_id, cmd(), CameraManagerDoneStatus.ERROR);
                return;
            }

            client.onStreamStart(reason);
