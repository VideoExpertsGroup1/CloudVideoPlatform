...
class WebSocketReceiver {
            ...
            try {
                ...
                if (rsv) {
                    ...
                    if (opcode == WebSocket.OPCODE_CLOSE) {
                        ...
                        Log.v("WS_PING_PONG", "OPCODE_PONG receive (NOTE: as a client, we don't expect PONGs. No-op");
                ...
                handleError(new WebSocketException("SocketTimeoutException", sto));
    ...
    private void appendBytes(boolean fin, byte opcode, byte[] data) {
        ...
        if (opcode == WebSocket.OPCODE_PING) {
            Log.v("WS_PING_PONG", "OPCODE_PING receive");
            if (fin) {
                handlePing(data);
            } else {
